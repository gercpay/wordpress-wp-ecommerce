 ...
 * Class GercPay
 */
class GercPay extends wpsc_merchant {
	...
	/**
	 * GercPay instance existence flag.
	 *
	 * @var bool
	 */
	...
	/**
	 * GercPay payment page locale.
	 *
	 * @var string
	 */
	protected $gercpay_locale;
	...
	/**
	 * Returns instance of GercPay.
	 *
	 * @return bool|gercpay
	 */
	public static function getInst() {
		if ( false === self::$inst ) {
			self::$inst = new GercPay();
		}
	...
	protected function generate_form( $data ) {
		$form = '<form method="post" id="form_gercpay" action="' . $this->url . '" accept-charset="utf-8">';
		foreach ( $data as $k => $v ) {
	...
	public function check_response( $response ) {
		...
		$ref = get_bloginfo() . '_' . md5( 'gercpay_' . $session_id );

		$purchase_log = new WPSC_Purchase_Log( $session_id, 'sessionid' );
		if ( ! $purchase_log->exists() || $purchase_log->is_transaction_completed() ) {
			return esc_html__( 'An error has occurred during payment. Please contact us to ensure your order has submitted.', 'gercpay-for-wp-ecommerce' );
		}

		if ( sanitize_text_field( get_option( 'gercpay_merchant_id' ) ) !== $response['merchantAccount'] ) {
			return esc_html__( 'An error has occurred during payment. Merchant data is incorrect.', 'gercpay-for-woocommerce' );
		}
		...
		if ( empty( $response['merchantSignature'] ) || $response['merchantSignature'] !== $response_signature ) {
			die( esc_html__( 'An error has occurred during payment. Signature is not valid.', 'gercpay-for-wp-ecommerce' ) );
		}
	...
	/**
	 * Return Gercpay payment page locale.
	 *
	 * @return string
	 */
	public function get_gercpay_locale() {
		return $this->gercpay_locale;
	}

	/**
	 * Set Gercpay payment page locale.
	 *
	 * @return void
	 * @param string $locale Language code.
	 */
	public function set_gercpay_locale( $locale ) {
		if ( in_array( $locale, self::GERCPAY_ALLOWED_LOCALES, true ) ) {
			$this->gercpay_locale = $locale;
		}
	}
}
